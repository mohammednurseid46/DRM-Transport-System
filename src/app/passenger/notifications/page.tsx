...
export default function PassengerNotificationsPage() {
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem('passengerNotifications');
    
    if (stored) {
      const parsed = JSON.parse(stored);
      if (parsed.length > 0) {
        // Ensure they have a category (backward compatibility with layout mock data)
        const mapped = parsed.map((n: any) => ({
          ...n,
          category: n.category || (
            n.title.toLowerCase().includes("ride") || n.title.toLowerCase().includes("driver") ? "Trips" :
            n.title.toLowerCase().includes("discount") || n.title.toLowerCase().includes("payment") ? "Payments & Splits" :
            "Safety & System"
          )
        }));
    ...
    localStorage.setItem('passengerNotifications', JSON.stringify(initialNotifications));
    setIsLoading(false);
  }, [router]);

  const saveNotifications = (newNotifications: Notification[]) => {
    setNotifications(newNotifications);
    localStorage.setItem('passengerNotifications', JSON.stringify(newNotifications));
    // Trigger custom event so layout.tsx can update its unread count if it was listening (optional enhancement)

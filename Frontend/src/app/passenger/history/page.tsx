...
export default function PassengerHistoryPage() {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const data = await api.get('/rides/user');
        // Map the backend response to the frontend's expected format if needed
        const mappedRides = data.map((ride: any) => ({
          id: ride.ride_id,
          rideType: ride.ride_type,
          status: ride.status,
          pickup: ride.pickup_landmark,
          destination: ride.dropoff_landmark,
          fare: ride.base_fare,
          paymentMethod: ride.fare_type,
          rating: 5, // mock or get from review
          completedAt: ride.created_at
        }));

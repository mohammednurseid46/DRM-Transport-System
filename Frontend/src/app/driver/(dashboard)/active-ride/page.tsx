...
export default function DriverActiveRidePage() {
  ...
  useEffect(() => {
    const fetchActiveRide = async () => {
      const activeRideId = localStorage.getItem("driverActiveRideId");
      if (activeRideId) {
        try {
          const rideData = await api.get(`/rides/${activeRideId}`);
          setRide(rideData);
        } catch (err) {
    ...
    // Poll for updates in case passenger cancels, etc.
  ...
  const updateRideStatus = async (newStatus: string) => {
    try {
      ...
      // EN_ROUTE -> ACCEPTED, IN_PROGRESS -> IN_PROGRESS, COMPLETED -> COMPLETED, ARRIVED -> ARRIVED (assuming enum supports these)
      // The schema says ENUM('PENDING', 'ACCEPTED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED')
      if (newStatus === "arrived") backendStatus = "ACCEPTED"; // Or maybe "IN_PROGRESS" - let's map "arrived" to a local state or "ACCEPTED"
      if (newStatus === "completed") backendStatus = "COMPLETED";
      if (newStatus === "in_progress") backendStatus = "IN_PROGRESS";
      
      await api.patch(`/rides/${ride.ride_id}/status`, { status: backendStatus });
  ...
  const getActionText = () => {
    // Map local frontend status or backend status
    const stat = ride.status.toLowerCase();
    switch (stat) {
      case "accepted":
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <svg className="w-full h-full max-w-lg" viewBox="0 0 400 400">
          ...
          <g className={`transition-all duration-1000 ${
            (ride.status.toLowerCase() === "accepted" || ride.status.toLowerCase() === "en_route") ? "translate-x-[20px] translate-y-[350px]" : 
            ride.status.toLowerCase() === "arrived" ? "translate-x-[100px] translate-y-[300px]" : 
            ride.status.toLowerCase() === "in_progress" ? "translate-x-[200px] translate-y-[200px]" : 
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="text-xl font-bold text-slate-900 dark:text-slate-900 dark:text-slate-900 dark:text-white mb-0.5">
                {(ride.status.toLowerCase() === "accepted" || ride.status.toLowerCase() === "en_route" || ride.status.toLowerCase() === "arrived") 
                  ? ride.pickup_landmark 
                  : ride.dropoff_landmark}
              </div>
      ...
      <div className="absolute bottom-0 left-0 w-full z-20">
        <div className="bg-slate-800 rounded-t-[32px] p-6 shadow-[0_-10px_40px_rgba(0,0,0,0.3)] border-t border-slate-700 max-w-4xl mx-auto flex flex-col gap-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 bg-slate-700 rounded-full flex items-center justify-center text-2xl font-bold text-slate-400">
                {ride.passengers?.[0]?.user?.full_name ? ride.passengers[0].user.full_name.charAt(0) : "P"}
              </div>
              <div>
                <h3 className="text-lg font-bold text-slate-900 dark:text-slate-900 dark:text-slate-900 dark:text-white mb-1">{ride.passengers?.[0]?.user?.full_name || "Passenger"}</h3>
                <div className="flex items-center gap-1.5 text-xs font-mono text-slate-400 bg-slate-900/50 px-2 py-1 rounded border border-slate-700 w-fit">
                  ...
                  {ride.passengers?.[0]?.user?.phone_number || "+251911..."}
                </div>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-2xl font-black text-slate-900 dark:text-slate-900 dark:text-slate-900 dark:text-white">{ride.base_fare?.toFixed(2) || "0.00"} ETB</div>
          </div>
              ...
              <button 
                onClick={() => { 
                  ...
                  const sosData = {
                    ...
                    triggeredBy: `Driver: ${ride.driver?.user?.full_name || "Unknown"}`,
                    role: "driver",
                    phone: currentUser.phone || "Unknown",
                    plateNumber: ride.driver?.vehicle?.plate_number || "Unknown",
                    location: ride.pickup_landmark || "Unknown Route",
                    landmark: ride.pickup_landmark,
                    userRole: "driver",

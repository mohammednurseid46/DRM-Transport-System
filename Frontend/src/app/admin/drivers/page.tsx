...
export default function DriverManagementPage() {
  ...
  const pendingDrivers = drivers.filter(d => d.status === "pending" || (!d.is_verified && d.status !== 'online' && d.status !== 'offline'));
  const verifiedDrivers = drivers.filter(d => d.status === "online" || d.status === "offline" || d.is_verified);
  const activeOnlineCount = verifiedDrivers.filter(d => d.status === "online" || d.isAvailable).length;
            ...
            {currentList.map(driver => (
              <div key={driver.id} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-5 flex flex-col gap-4 relative overflow-hidden group">
                ...
                <div className="absolute top-4 right-4">
                  {driver.status === 'online' || driver.status === 'offline' || driver.is_verified ? (
                ...
                <div className="grid grid-cols-2 gap-2 p-3 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-200 dark:border-slate-700">
                  <div>
                    ...
                    <p className="text-sm font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1"><Car size={14} className="text-orange-600 dark:text-orange-500" /> {driver.vehicleInfo || driver.vehicleModel || "Unknown"}</p>
                  </div>
                  <div>
                  ...
                  <div className="col-span-2 grid grid-cols-2 pt-2 border-t border-slate-200 dark:border-slate-700 mt-1 gap-2">
                    ...
                    {driver.status !== 'pending' && (
                      <div>
                        ...
                        <p className="text-sm font-bold text-slate-900 dark:text-slate-900 dark:text-slate-900 dark:text-white flex items-center gap-1"><Star size={14} className="text-yellow-500 fill-yellow-500" /> {driver.rating || (Math.random() * 1.5 + 3.5).toFixed(1)}</p>
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  <div className="flex flex-col gap-3 pt-2 mt-2 border-t border-slate-200 dark:border-slate-700">
                     <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-900/50 p-2 rounded-lg">
                       ...
                       >
                         ...
                         {driver.status === 'online' || driver.isAvailable ? <ToggleRight size={28} /> : <ToggleLeft size={28} />}
                       </button>
                     </div>

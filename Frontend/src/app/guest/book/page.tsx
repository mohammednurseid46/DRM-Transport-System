...
export default function GuestBookRidePage() {
  ...
  const handleBookRide = async () => {
    if (!selectedProduct || !pickup || !destination || !phoneNumber) return;
    
    setIsBooking(true);
    
    try {
      // Helper to map landmarks to actual Bahir Dar coordinates (re-implement here since guest page might not have it)
  ...
  const handleConfirmRide = (driverId: string) => {
    // Store ride info in localStorage for the active trip view
    const driver = availableDrivers.find(d => d.driver_id === driverId) || { driver_id: driverId, user: { full_name: 'Unknown' }, vehicle: { model: 'Unknown' } };
    if (!driver) return;

    const rideData = {
      rideId: `ride_${Date.now()}`,
      passengerName: "Guest Passenger",
      passengerPhoneMasked: phoneNumber,
      ...
      driver: driver, // passing this inside for simplicity
      timestamp: new Date().toISOString()
    };
    
    // Save to activeRides which the driver dashboard will listen for
    localStorage.setItem("activeRides", JSON.stringify(rideData));
  ...
  useEffect(() => {
    ...
    const checkRideStatus = () => {
      ...
      if (active) {
        const ride = JSON.parse(active);
        if (ride.status === "accepted" || ride.status === "en_route") {
          // Driver accepted, copy to currentActiveRide for the active trip view
          localStorage.setItem("currentActiveRide", JSON.stringify(ride));
          // Store a flag to show the toast on the next page
          sessionStorage.setItem("showAcceptedToast", "true");
          router.push("/guest/active-trip");
        } else if (ride.status === "declined") {
          setIsWaitingForDriver(false);
          localStorage.removeItem("activeRides");
          alert("No drivers available at the moment. Please try again.");
        }
      }
    };

    const interval = setInterval(checkRideStatus, 1000);
    window.addEventListener("storage", checkRideStatus);
    
    return () => {
      clearInterval(interval);
      window.removeEventListener("storage", checkRideStatus);
    };
        ...
        <div className="lg:col-span-5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-card p-6 flex flex-col">
          ...
          <div className="space-y-6 flex-1">
            ...
            <div className="flex bg-slate-100 dark:bg-slate-700 rounded-lg p-1">
              ...
              >
                <Map size={14} /> Landmarks
              </button>
            </div>

            {/* Phone Number for Guest */}
            <div className="relative">
              <label className="block text-[11px] font-bold tracking-widest text-slate-500 dark:text-slate-400 uppercase mb-2">Phone Number</label>
              <div className="relative">
                <input 
                  type="tel" 
                  value={phoneNumber}
                  onChange={(e) => setPhoneNumber(e.target.value)}
                  className="w-full bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-900 dark:text-slate-900 dark:text-white text-sm rounded-control px-4 py-3 focus:outline-none focus:border-orange-500/50 transition-colors placeholder-slate-400 dark:placeholder-slate-500"
          ...
          <button 
            ...
            disabled={isBooking || !pickup || !destination || !phoneNumber}
            className={`w-full mt-6 font-bold py-3.5 rounded-control flex items-center justify-center gap-2 transition-colors shadow-lg ${
              isBooking || !pickup || !destination || !phoneNumber 
        ...
        <div className="lg:col-span-7 bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-card relative overflow-hidden min-h-[400px]">
          ...
          <div className="absolute left-6 top-6 z-10">
            <div className="bg-white dark:bg-slate-800/90 backdrop-blur border border-slate-200 dark:border-slate-700 rounded-badge px-4 py-2 flex items-center gap-2 shadow-lg">
              ...
              <span className="text-xs font-medium text-slate-900 dark:text-slate-900 dark:text-slate-900 dark:text-slate-900 dark:text-white">48 Drivers active nearby</span>
            </div>
          </div>
        </div>
      </div>
